...
const putAuthToken = (instance: AxiosInstance) => {
	...
	instance.interceptors.request.use(
		(req) => {
			...
			['tutorials', 'excercises', 'profile', 'classrooms', 'compilation', 'account', 'exercise'].some((item) =>
...
export function post(url: string, requestData: Record<string, unknown>): Promise<AxiosResponse<any>> {
	const instance = returnAxiosInstance();
	!url.includes('login') && putAuthToken(instance);
	return instance.post(url, requestData);
}

export function put(url: string, requestData: Record<string, unknown>): Promise<AxiosResponse<any>> {
	const instance = returnAxiosInstance();
	putAuthToken(instance);
